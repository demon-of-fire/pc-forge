#!/usr/bin/env python3
"""Initial full data population for PCForge hardware database."""
...
logger = logging.getLogger("initial_populate")
...
def deduplicate_category(components: list[dict], category: str) -> list[dict]:
    ...
    logger.info("Removed %d duplicates from %s", len(to_remove), category)
    return deduplicated
...
def run_population(
    ...
    for category in categories_to_run:
        ...
        logger.info("Processed %d items for %s", len(processed), category)

        processed = deduplicate_category(processed, category)

        data_file = DATA_DIR / f"{category}.json"

        if dry_run:
            logger.info("[DRY RUN] Would save %d items to %s", len(processed), data_file)
            if data_file.exists():
                result = validate_file(str(data_file))
                logger.info("Existing file validation: %s", "PASS" if result["valid"] else "FAIL")
        else:
            DATA_DIR.mkdir(parents=True, exist_ok=True)
            processor.save(processed, data_file)
            ...
            all_changes["added"][category] = processed
...
def main():
    parser = argparse.ArgumentParser(description="PCForge: Initial data population")
    parser.add_argument(
        "--category", "-c",
